      ...
      <div className="max-w-5xl mx-auto px-5 py-6">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3.5">
            ...
            <div className="text-xl font-semibold text-white tabular-nums">{tickers.length}</div>
          </div>
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl px-4 py-3.5">
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            filtered.map((ticker) => (
              <TickerRow key={ticker.symbol} ticker={ticker} onClick={() => navigate(`/markets/ticker/${ticker.symbol}`)} />
            ))
          )}
        </div>

        {!loading && filtered.length === 0 && (
          <div className="text-center py-20 text-white/20 font-mono text-sm">No tickers found</div>
        )}
...
function TickerRow({ ticker, onClick }: { ticker: DashboardTicker; onClick: () => void }) {
