    ...
    <div className="phone-container min-h-screen flex flex-col bg-background">
      <div className="flex-1 px-7 pt-12 pb-10 flex flex-col">
        <motion.div className="text-center mb-10" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <div className="w-24 h-24 rounded-full border-2 border-primary/30 mx-auto mb-5 flex items-center justify-center"><span className="font-serif text-[36px] text-foreground">{result.alignment[0]}</span></div>
            ...
            return (
              <motion.div key={key} className="mb-5 last:mb-0" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 + i * 0.1 }}>
                ...
                <div className="h-[6px] bg-foreground/8 rounded-full relative overflow-hidden">
                  ...
                  <motion.div className="absolute top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-primary/80 border border-primary/40"
        ...
        <div className="mt-auto flex gap-3 pt-8">
          ...
          <button onClick={() => navigate("/dilemma")} className="flex-1 py-3.5 rounded-xl bg-primary text-primary-foreground text-sm font-medium">Retake Quiz</button>
        </div>
      </div>
    </div>
  );
