  ...
  return (
    <div className={`phone-container min-h-screen flex flex-col bg-background relative overflow-hidden ${isStoic ? "stoic-grain" : ""} ${isOcean ? "ocean-shimmer" : ""}`}>
      ...
      <div className="px-7 pt-3 pb-6 relative z-10">
        {isStoic ? (
          <>
            <h2 className="stoic-text text-[28px] text-white tracking-[0.04em]">CHOOSE YOUR ARENA</h2>
            <p className="text-[10px] text-white/25 tracking-[0.15em] uppercase">{philosopher ? `DEBATING ${philosopher.name.toUpperCase()}` : "WHAT WILL YOU DEFEND TODAY"}</p>
          </>
        ) : (
          <>
      ...
      <div className="flex-1 px-7 grid grid-cols-2 gap-3 content-start relative z-10 pb-4">
        {topics.map((topic, i) => (
          <motion.button key={topic.id} onClick={() => navigate(`/arena/arenas/${philosopherId}/${topic.id}`)}
            ...
            {isStoic ? (
              <>
                <h4 className="stoic-text text-[16px] text-white tracking-[0.03em]">{topic.name.toUpperCase()}</h4>
                <span className="text-[9px] text-white/20 uppercase tracking-[0.12em]">{topic.subtitle}</span>
              </>
            ) : (
              <>
